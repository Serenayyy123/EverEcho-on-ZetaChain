...
async function checkEncryptionPubKeyStatus() {
  ...
  try {
    ...
    if (allTasks.length > 0) {
      const tasksWithEncryption = allTasks.filter(t => 
        t.contactsEncryptedPayload && 
        t.creatorWrappedDEK && 
        (t.helperWrappedDEK || !t.helper || t.helper === '0x0000000000000000000000000000000000000000')
      );
      
      const tasksWithoutEncryption = allTasks.filter(t => 
        !t.contactsEncryptedPayload || !t.creatorWrappedDEK
      );

      console.log(`✅ 正确加密的Task: ${tasksWithEncryption.length}`);
      console.log(`❌ 缺少加密数据的Task: ${tasksWithoutEncryption.length}`);
      ...
      if (tasksWithoutEncryption.length > 0) {
        ...
        tasksWithoutEncryption.forEach((task, index) => {
          ...
          console.log(`   Helper: ${task.helper || '(无)'}`);
          console.log(`   contactsEncryptedPayload: ${task.contactsEncryptedPayload ? '有' : '无'}`);
          console.log(`   creatorWrappedDEK: ${task.creatorWrappedDEK ? '有' : '无'}`);
          console.log(`   helperWrappedDEK: ${task.helperWrappedDEK ? '有' : '无'}`);
          console.log('');
        });
      }
    }
